...
const EntityAnnotator = ({ 
  ...
  const validateAnnotation = useCallback((annotation, plainText) => {
    ...
    if (typeof start !== 'number' || typeof end !== 'number') {
      console.warn('标注位置类型错误:', annotation);
      return false;
    }
    ...
    const actualText = plainText.slice(start, end);
    if (!actualText || actualText.trim().length === 0) {
      console.warn('标注文本为空:', annotation);
      return false;
    }
            ...
            {!readOnly && (
              <div className="annotation-actions">
                <button
                  ...
                  type="button"
                >
                  {typeof window !== 'undefined' && window.feather ? (
  ...
  const renderAnnotatedText = () => {
    try {
      ...
      sortedAnnotations.forEach((annotation, annotationIndex) => {
        if (!annotation || !validateAnnotation(annotation, plainText)) {
          console.warn('无效的标注，跳过:', annotation);
          return;
        }

        // 确保 start 和 end 在合理范围内
        const safeStart = Math.max(0, Math.min(annotation.start, plainText.length));
        const safeEnd = Math.max(safeStart, Math.min(annotation.end, plainText.length));
        
        if (safeStart > lastIndex) {
          const textSegment = plainText.slice(lastIndex, safeStart);
        ...
        const actualText = plainText.slice(safeStart, safeEnd);
        ...
        annotationLines.forEach((line, lineIndex) => {
          ...
          if (displayLine.trim()) {
            ...
            // 使用更稳定的 key
            const annotationKey = annotation.id 
              ? `annotation-${annotation.id}-${lineIndex}-${safeStart}`
              : `annotation-${annotationIndex}-${lineIndex}-${safeStart}-${elementCounter++}`;
      ...
      console.error('渲染标注文本时出错:', error);
      return <span className="error-text">渲染错误，请检查标注数据</span>;
    }
  };
